...
public class StandardEnforcementPolicy implements EnforcementPolicy {
    ...
    public StandardEnforcementPolicy(Collection<TargetAnnotation> targetAnnotations, Filter<ClassInfo> safetyFilter) {
        this.targets = targetAnnotations.stream()
        ...
        method.findAttribute(Attributes.runtimeVisibleTypeAnnotations())
                .ifPresent(attr -> {
                    for (TypeAnnotation ta : attr.annotations()) {
                        if (ta.targetInfo() instanceof TypeAnnotation.FormalParameterTarget pt && pt.formalParameterIndex() == paramIndex) {
    ...
    private List<Annotation> getFieldAnnotations(FieldModel field) {
        ...
        field.findAttribute(Attributes.runtimeVisibleTypeAnnotations())
             .ifPresent(attr -> {
                 for (TypeAnnotation ta : attr.annotations()) {
